...
Loads the Jaeger agent ONCE, then starts every channel adapter that
...
instance, the same memory store, and the same skill / cron state.
  ...
  Telegram adapter:  needs TELEGRAM_BOT_TOKEN, optional TELEGRAM_ALLOWED_USER_IDS
  iMessage adapter:  needs IMESSAGE_ALLOWED_HANDLES (and Full Disk Access)

Run:
    python -m python_jaeger.plugins.messaging.gateway
    python -m python_jaeger.plugins.messaging.gateway --no-imessage
    python -m python_jaeger.plugins.messaging.gateway --no-discord

Mirrors python_pydantic_ai.plugins.messaging.gateway — the only deltas
are the imports (jaeger's main API) and the LlamaCppPythonClient call
(jaeger takes a ModelConfig, pydantic_ai takes raw kwargs).
"""
...
def main() -> int:
    ...
    if not layout.exists():
        print(f"[gateway] instance {instance_name!r} not initialized; "
              f"run `python -m python_jaeger --setup` first.", file=sys.stderr)
        return 2
