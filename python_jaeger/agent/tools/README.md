...
`from python_jaeger.agent.tools import my_new_tool`, then wire it onto
the agent in `python_jaeger/main.py`:
...
| `files.py` | `file_write`, `file_read`, `list_skill_dir` (sandboxed to skills/) |
